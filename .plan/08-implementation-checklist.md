...
- Last verification run: `cargo fmt --check`, `cargo test`, `cargo check` (pass on 2026-06-26 after baking PAGEDISCARD into the module-files install path and adding regression coverage for file-backed install/upgrade/repair/restore activation via page discard instead of page store)`
