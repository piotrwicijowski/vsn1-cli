...
- Last verification run: `cargo fmt --check`, `cargo test`, `cargo check` (pass on 2026-06-26 after converting the bundled media runtime to the module-files backend and keeping the existing PAGEDISCARD-based file-backed activation path green in regression coverage)`
